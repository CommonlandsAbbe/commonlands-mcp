...
| `read_shopify_products` | Live Shopify product truth (public data only). | At least one of `sku`, `handle`, or `query` should be supplied for useful results. | `limit` 1-25 variant records, `includeMetafields` true/false (default false; allowlisted `custom.*` display fields only). | `shopify.live_read.v1` with live Product/Variant GIDs, SKU, price, coarse `availability`, product URL, media, allowlisted metafields when requested, read-only safety flags. ACTIVE products only; no exact inventory counts. A connector message warns when the requested limit is reached and the variant set may be incomplete. | Essential before final purchasable claims or cart handoff. This is the main truth tool. |
